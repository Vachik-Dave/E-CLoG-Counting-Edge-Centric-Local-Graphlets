...
class graph
{
// all member variables are public, so it can be directly accessed.
public:
	NodeID_TYPE N,M;							// N = number of vertices ; M = number of edges
	vector<NodeID_TYPE> * adj_list;						// adjacency list
	...
	// initializer in case of default constructor is used
...
//*/
	...
	// sort node_ids (neighbors) of each node in adjacency list format
	double sort_edges()
	{
		Runtimecounter rt1;
		rt1.start();
	...
	// converting to row major (CSR) compressed representation
